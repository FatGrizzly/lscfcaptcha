...
func verifyResp(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("Failed to extract Cloudflare's response. %+v", err)
		return
	}
	...
	if err != nil {
		log.Printf("Cloudflare's response is malformed. %+v", err)
		return
	}

	if !recaptcha.Success {
		log.Printf("Captcha not ok, errors %+v", recaptcha.ErrorCodes)
		return
	}
